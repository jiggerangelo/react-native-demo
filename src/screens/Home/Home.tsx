...
const Home = ({ navigation }: IProps) => {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <TouchableOpacity
          onPress={() => navigation.navigate('Detail', { current: true })}
          style={styles.currentSection}
        >
          <View style={styles.left}></View>
          <View style={styles.middle}>
            <MyAppText style={styles.currentDate}>
              Today, {getMonthName(new Date(currentWeather.dt * 1000).getMonth())}{' '}
          ...
          <View style={styles.left}></View>
          <View style={styles.middle}>
            <MyAppText style={styles.currentFeelsLike}>
              {Math.round(parseInt(currentWeather.feels_like))}&deg;
            </MyAppText>
            <MyAppText style={styles.currentTemperature}>{Math.round(parseInt(currentWeather.temp))}&deg;</MyAppText>
          </View>
          <View style={styles.right}>
            <Image style={styles.currentTempImage} source={FORECAST_ICON[currentWeather.weather[0].main]} />
        ...
        {weatherList &&
          weatherList.map(
            (item: any, index: number) =>
              // Skip first item because it's the same as current
              index !== 0 && (
                <TouchableOpacity
                  key={index}
                  onPress={() => navigation.navigate('Detail', { index: index })}
                  style={styles.row}
                >
